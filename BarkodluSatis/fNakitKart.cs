...
namespace BarkodluSatis
{
    public partial class fNakitKart : Form
    {
        ...
        private void tNakit_KeyDown(object sender, KeyEventArgs e)
        {
            if (tNakit.Text != "")
            {
                if (e.KeyCode == Keys.Enter) //açılmış formun label ına değer gönderiyoruz.
                {
                    Hesapla();
                }
            }           
        ...
        {
            if(char.IsDigit(e.KeyChar)==false && e.KeyChar!=(char)08) //karakter veya harf yazamasın, 08 silme tuşunun ascii kodu
            {
                e.Handled = true;
            }
        }
